...
export async function runCoachTurn(
  ...
  /**
   * The code-execution container this turn is using, if it acquired one.
   *
   * `web_search_20260209` filters its own results by running code server-side,
   * and that code runs in a container. Once a turn has done that, every later
   * request in the same turn has to say which container the conversation
   * belongs to — otherwise the API rejects it outright:
   *
   *   400 container_id is required when there are pending tool uses
   *       generated by code execution with tools.
   *
   * Which is what happened the first time the coach was asked to look up a
   * sandwich it didn't know: it searched, the search ran code, the loop sent
   * the tool results back without the container, and the whole turn died with
   * a raw API error in the thread.
   *
   * Turn-scoped rather than loop-scoped on purpose — the verification pass
   * re-enters `runToolLoop` with this same message list, so the container has
   * to travel with it. Containers outlive a turn by a long way, so nothing
   * here needs to think about `expires_at`.
   */
  ...
  async function runToolLoop({ forceTool = false } = {}) {
    ...
    for (let i = 0; i < MAX_ITERATIONS; i++) {
      try {
        // Beta namespace: `container` is only accepted there. No beta flag is
        // needed for anything this call uses — the server tools and effort are
        // both GA — and the rendered prompt prefix is unchanged, so the cache
        // is not affected.
        response = await anthropic.beta.messages.create({
          ...
          // Only once the turn actually has one. Sending `container: null` is
      ...
      // Carry the container forward before anything can `continue` past it.
